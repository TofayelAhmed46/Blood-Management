<x-frontend.layouts.partials.main>
    <div class="container-fluid">
        <div class="row">
            <div class="card-group">

                @foreach ($data as $datas )
                    
               
                <div class="card cardMargin">
                    <img src="{{asset('ui/frontend/assets/image')}}/{{$datas->donar==null ? '':$datas->donar->image}}"  class="card-img-top" alt="No Image">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>

                @endforeach
               
            </div>
        </div>

        <div class="row">
            <div class="card-group">
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
            </div>
        </div>



        <div class="row mt-5">
            <div class="card-group">
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
                <div class="card cardMargin">
                    <img src="{{ asset('ui/frontend') }}/assets/image/donar/donar1.jpg" class="card-img-top"
                        alt="...">
                    <div class="card-body">
                        <h5 class="card-title">Card title</h5>
                        <p class="card-text">This is a wider card with supporting .</p>
                        <div class="btn donar-btnview">
                            <a href="{{ route('donarprofile') }}">View Details</a>
                        </div>
                        <div class="social-link">
                            <ul class="social-img">
                                {{-- <li><a href="#"><i class="fa-brands fa-facebook-f" style="color: #c4bebe"></i></a></li> --}}
                                <li><a href="#"><i class="fa-brands fa-facebook-f"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-twitter"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-linkedin"></i></a></li>
                                <li><a href="#"><i class="fa-brands fa-google-plus"></i></a></li>
                            </ul>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </div>

    </div>










</x-frontend.layouts.partials.main>
